function x0 = bfgs(x0, f, g, X1, X2, Z, debug)
    H_k = {eye(2)};
    g0 = g(x0);
    g_k = {g0};
    x_k = {x0};
    for k = 1:2
        if g_k{k} ~= 0
            ...
            if g_k{k+1} ~= 0
                ...
                H_k{k+1} = H_k{k} + ...
                    (1 + ((del_g{k}.')*H_k{k}*del_g{k})/((del_g{k}.')*del_x{k}))*((del_x{k}*(del_x{k}.'))/((del_g{k}.')*del_x{k})) - ...
                    ((H_k{k}*del_g{k}*(del_x{k}.') + (H_k{k}*del_g{k}*(del_x{k}.')).')/((del_g{k}.')*del_x{k}));
                k = k+1;
            end
        end
    end
